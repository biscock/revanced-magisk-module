...
get_prebuilts() {
	...
	for src_ver in "$cli_src CLI $cli_ver cli" "$patches_src Patches $patches_ver patches"; do
		...
		if [ "$tag" = "Patches" ] && [ "$src" = "ReVanced/revanced-patches" ]; then
			local rvapi_resp rvp_url rvp_ver rvp_name
			rvapi_resp=$(curl -sf "https://api.revanced.app/v5/patches") || return 1
			rvp_url=$(jq -r '.download_url' <<<"$rvapi_resp")
			rvp_ver=$(jq -r '.version' <<<"$rvapi_resp")
			rvp_name="patches-${rvp_ver#v}.rvp"
			file="${dir}/${rvp_name}"
			if [ ! -f "$file" ]; then
				gh_dl "$file" "$rvp_url" >&2 || return 1
			fi
			echo "Patches: ReVanced/${rvp_name}  " >>"${cl_dir}/changelog.md"
			echo -n "$file "
			continue
		fi
